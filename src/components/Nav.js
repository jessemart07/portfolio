...
function Nav() {
  ...
  return (
    <nav className='flex flex-col items-center xl:justify-center h-max gap-y-4 fixed bottom-0
    mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen'>
        ...
        xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] py-8 bg-accent/10 xl:h-max backdrop-blur-sm 
        text-3xl xl:text-xl xl:rounded-full'>
            {navData.map((link, index) => {
                return (
                    <Link className={`${link.path === pathname && 'text-secondary'}
                     relative flex items-center group hover:text-accent transition-all duration-300`} href={link.path} key={index}>
                        {/* Tooltip */}
